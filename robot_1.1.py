...
def deplacement():
	...
	while 1:
		...
		for event in events:
			if event.ev_type == 'Absolute' and event.code in ['ABS_RZ','ABS_Z','ABS_X','ABS_Y']:
			...
			if (event.ev_type, event.code) == ('Absolute', 'ABS_RY'):
				...
				if valeur <= 0:
					...
					position = pos_init_ballon - ((pos_avant_ballon - pos_init_ballon) * valeur) // 32640
				
				else:
					
					position = pos_init_ballon - ((pos_init_ballon - pos_arriere_ballon) * valeur) // 32640
			
				try:
					r.ballon.move(position)
				except:
					pass
